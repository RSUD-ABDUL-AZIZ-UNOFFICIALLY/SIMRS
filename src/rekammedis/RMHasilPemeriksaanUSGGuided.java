 ...
 * @author perpustakaan
 */
public final class RMHasilPemeriksaanUSGGuided extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public RMHasilPemeriksaanUSGGuided(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        
        tabMode=new DefaultTableModel(null,new Object[]{
                "No.Rawat","No.RM","Nama Pasien","Tgl.Lahir","Kode Dokter","Nama Dokter","Tanggal","Kiriman Dari","Diagnosa Klinis","Kesimpulan"
            }){
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Hasil Pemeriksaan USG Guided ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 11), new java.awt.Color(50, 50, 50))); // NOI18N
        ...
        Tanggal.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "21-08-2024 09:24:32" }));
        ...
        jLabel48.setBounds(0, 120, 80, 23);
        ...
        scrollPane21.setBounds(90, 120, 635, 510);
    ...
    private void MnPenilaianMedisActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnPenilaianMedisActionPerformed
        if(tbObat.getSelectedRow()>-1){
            ...
            Valid.MyReportqry("rptCetakHasilPemeriksaanUSGGuided.jasper","report","::[ Formulir Hasil Pemeriksaan USG ]::",
                "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,pasien.tgl_lahir,pasien.pekerjaan,pasien.alamat,hasil_pemeriksaan_usg_guided.tanggal,"+
                "hasil_pemeriksaan_usg_guided.kd_dokter,dokter.nm_dokter,hasil_pemeriksaan_usg_guided.diagnosa_klinis,hasil_pemeriksaan_usg_guided.kiriman_dari,"+
                "hasil_pemeriksaan_usg_guided.kesimpulan from reg_periksa inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                "inner join hasil_pemeriksaan_usg_guided on reg_periksa.no_rawat=hasil_pemeriksaan_usg_guided.no_rawat "+
                "inner join dokter on hasil_pemeriksaan_usg_guided.kd_dokter=dokter.kd_dokter where hasil_pemeriksaan_usg_guided.no_rawat='"+tbObat.getValueAt(tbObat.getSelectedRow(),0).toString()+"'",param);
        }
    ...
    private void btnAmbilActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAmbilActionPerformed
        if(tabMode.getRowCount()==0){
            ...
            if(tbObat.getSelectedRow()>-1){
                this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                Valid.panggilUrl("hasilpemeriksaanusgguided/login.php?act=login&usere="+koneksiDB.USERHYBRIDWEB()+"&passwordte="+koneksiDB.PASHYBRIDWEB()+"&no_rawat="+tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
                this.setCursor(Cursor.getDefaultCursor()); 
            }else{
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            RMHasilPemeriksaanUSGGuided dialog = new RMHasilPemeriksaanUSGGuided(new javax.swing.JFrame(), true);
        ...
        try{
            if(TCari.getText().trim().equals("")){
                ps=koneksi.prepareStatement(
                        "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,pasien.tgl_lahir,hasil_pemeriksaan_usg_guided.tanggal,"+
                        "hasil_pemeriksaan_usg_guided.kd_dokter,dokter.nm_dokter,hasil_pemeriksaan_usg_guided.diagnosa_klinis,hasil_pemeriksaan_usg_guided.kiriman_dari,"+
                        "hasil_pemeriksaan_usg_guided.kesimpulan from reg_periksa inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                        "inner join hasil_pemeriksaan_usg_guided on reg_periksa.no_rawat=hasil_pemeriksaan_usg_guided.no_rawat "+
                        "inner join dokter on hasil_pemeriksaan_usg_guided.kd_dokter=dokter.kd_dokter where "+
                        "hasil_pemeriksaan_usg_guided.tanggal between ? and ? order by hasil_pemeriksaan_usg_guided.tanggal");
            }else{
                ps=koneksi.prepareStatement(
                        "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,pasien.tgl_lahir,hasil_pemeriksaan_usg_guided.tanggal,"+
                        "hasil_pemeriksaan_usg_guided.kd_dokter,dokter.nm_dokter,hasil_pemeriksaan_usg_guided.diagnosa_klinis,hasil_pemeriksaan_usg_guided.kiriman_dari,"+
                        ...
                        "inner join hasil_pemeriksaan_usg_guided on reg_periksa.no_rawat=hasil_pemeriksaan_usg_guided.no_rawat "+
                        "inner join dokter on hasil_pemeriksaan_usg_guided.kd_dokter=dokter.kd_dokter where "+
                        "hasil_pemeriksaan_usg_guided.tanggal between ? and ? and (reg_periksa.no_rawat like ? or pasien.no_rkm_medis like ? or pasien.nm_pasien like ? or "+
                        "hasil_pemeriksaan_usg_guided.kd_dokter like ? or dokter.nm_dokter like ?) order by hasil_pemeriksaan_usg_guided.tanggal");
            }
    ...
    private void hapus() {
        if(Sequel.queryu2tf("delete from hasil_pemeriksaan_usg_guided where no_rawat=?",1,new String[]{
    ...
    private void ganti() {
        if(Sequel.mengedittf("hasil_pemeriksaan_usg_guided","no_rawat=?","no_rawat=?,tanggal=?,kd_dokter=?,diagnosa_klinis=?,kiriman_dari=?,kesimpulan=?",7,new String[]{
                ...
                KirimanDari.getText(),DiagnosaKlinis.getText(),Kesimpulan.getText(),tbObat.getValueAt(tbObat.getSelectedRow(),0).toString()
            })==true){
    ...
    private void panggilPhoto() {
        if(FormPhoto.isVisible()==true){
            try {
                ps=koneksi.prepareStatement("select hasil_pemeriksaan_usg_guided_gambar.photo from hasil_pemeriksaan_usg_guided_gambar where hasil_pemeriksaan_usg_guided_gambar.no_rawat=?");
                try {
                    ...
                    if(rs.next()){
                        if(rs.getString("photo").equals("")||rs.getString("photo").equals("-")){
                            ...
                            LoadHTML2.setText("<html><body><center><a href='http://"+koneksiDB.HOSTHYBRIDWEB()+":"+koneksiDB.PORTWEB()+"/"+koneksiDB.HYBRIDWEB()+"/hasilpemeriksaanusgguided/"+rs.getString("photo")+"'><img src='http://"+koneksiDB.HOSTHYBRIDWEB()+":"+koneksiDB.PORTWEB()+"/"+koneksiDB.HYBRIDWEB()+"/hasilpemeriksaanusgguided/"+rs.getString("photo")+"' alt='photo' width='550' height='550'/></a></center></body></html>");
                        }  
                    }else{
                        LoadHTML2.setText("<html><body><center><br><br><font face='tahoma' size='2' color='#434343'>Kosong</font></center></body></html>");
                    }
    ...
    private void simpan() {
        if(Sequel.menyimpantf("hasil_pemeriksaan_usg_guided","?,?,?,?,?,?","No.Rawat",6,new String[]{
                ...
                tabMode.addRow(new String[]{
                    TNoRw.getText(),TNoRM.getText(),TPasien.getText(),TglLahir.getText(),KdDokter.getText(),NmDokter.getText(),Valid.SetTgl(Tanggal.getSelectedItem()+"")+" "+Tanggal.getSelectedItem().toString().substring(11,19),
                    KirimanDari.getText(),DiagnosaKlinis.getText(),Kesimpulan.getText()
                });
                emptTeks();
                LCount.setText(""+tabMode.getRowCount());
        }
